...
public class BanCommand extends CommandBase {
    ...
    public void ban(@Sender CommandSender sender, Player player, String time, @OptArg @Text String reason) {
        ...
        XenonPlugin.getInstance().getShared().getRedis().sendPacket(new PunishmentUpdatePacket(sender instanceof Player ? ((Player) sender).getUniqueId() : XenonConstants.getConsoleUUID(), player.getUniqueId(), PunishmentType.BAN.name(), grant.getTimeCreated(), duration, reason, false, false));
    }
}
