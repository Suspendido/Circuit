...
public class ChatCommand extends BaseCommand {
    ...
    public void toggle(CommandSender sender) {
        ...
        XenonPlugin.getInstance().getShared().getRedis().sendPacket(new MessagePacket(XenonPlugin.getInstance().getShared().getServer().getName(), "&9Xenon &7» &f" + (ServiceContainer.getService(BukkitChatService.class).isChatEnabled() ? "&aChat was enabled!" : "&cChat was disabled!")));
    }
    ...
    public void cooldown(CommandSender sender, @Name("slowdown") long slowdown) {
        ...
        XenonPlugin.getInstance().getShared().getRedis().sendPacket(new MessagePacket(XenonPlugin.getInstance().getShared().getServer().getName(), "&9Xenon &7» &aChat slowdown was set to " + TimeUtils.formatTime(slowdown) + "!"));
    }
}
