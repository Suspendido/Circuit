...
public class UnmuteCommand extends BaseCommand {
    ...
    public void unmute(CommandSender sender, @Name("target") OfflinePlayer target, @Optional @Name("reason") @Flags("remaining") String reason) {
        ...
        if (profile.findActivePunishment(PunishmentType.MUTE) == null) {
            sender.sendMessage(CircuitConstants.getPlayerNotPunished().replace("<punishment_type>", PunishmentType.MUTE.getAction()));
            return;
        }
        ...
        CircuitPlugin.getInstance().getShared().getRedis().sendPacket(new PunishmentUpdatePacket(sender instanceof Player ? ((Player) sender).getUniqueId() : CircuitConstants.getConsoleUUID(), target.getUniqueId(), PunishmentType.MUTE.name(), -1, -1, reason, true, false));
    }
}
