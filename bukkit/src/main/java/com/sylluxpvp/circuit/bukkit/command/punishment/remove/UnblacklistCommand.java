...
public class UnblacklistCommand extends BaseCommand {
    ...
    public void unblacklist(CommandSender sender, @Name("target") OfflinePlayer target, @Optional @Name("reason") @Flags("remaining") String reason) {
        ...
        if (profile.findActivePunishment(PunishmentType.BLACKLIST) == null) {
            sender.sendMessage(CircuitConstants.getPlayerNotPunished().replace("<punishment_type>", PunishmentType.BLACKLIST.getAction()));
            return;
        }
        ...
        CircuitPlugin.getInstance().getShared().getRedis().sendPacket(new PunishmentUpdatePacket(sender instanceof Player ? ((Player) sender).getUniqueId() : CircuitConstants.getConsoleUUID(), target.getUniqueId(), PunishmentType.BLACKLIST.name(), -1, -1, reason, true, false));
    }
}
